CREATE TABLE `spider`.`books` ( `Id` INT(30) NOT NULL , `title` VARCHAR(30) NOT NULL , `author` VARCHAR(30) NOT NULL , `genre` VARCHAR(30) NOT NULL , `price` DECIMAL(20) NOT NULL , `copies_sold` INT(30) NOT NULL ) ENGINE = InnoDB;

INSERT INTO books (Id,title,author,genre,price,copies_sold) VALUES(1, 'The Silent Patient', 'Alex Michaelides', 'Thriller', 399.00, 1200)
(2, 'Atomic Habits', 'James Clear', 'Self-help', 499.00, 2000)
(3, 'The Psychology of Money', 'Morgan Housel', 'Finance', 350.00, 1800)
CREATE TABLE `spider`.`bestsellers` ( `Id` INT(30) NOT NULL , `title` VARCHAR(30) NOT NULL , `author` VARCHAR(30) NOT NULL , `genre` VARCHAR(30) NOT NULL , `price` DECIMAL(20) NOT NULL , `copies_sold` INT(30) NOT NULL ) ENGINE = InnoDB;
INSERT INTO bestsellers(Id,title,author,genre,price,copies_sold) VALUES((4, 'Ikigai', 'Francesc Miralles', 'Philosophy', 300.00, 2500)
(5, 'Think Like a Monk', 'Jay Shetty', 'Self-help', 450.00, 2200)

SELECT title, author FROM books UNION ALL SELECT title, author FROM bestsellers
SELECT * FROM books WHERE price>400
SELECT AVG(price) FROM books
SELECT COUNT(*) AS total_books FROM books
SELECT title AS "Book Title", author AS "Written By" FROM books